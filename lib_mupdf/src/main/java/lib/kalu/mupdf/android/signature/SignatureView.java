package lib.kalu.mupdf.android.signature;
...
public class SignatureView extends TextView {
    ...
    public String createSignatureBitmap() {
            ...
            return Executors.newSingleThreadExecutor().submit(new Callable<String>() {
                ...
                public String call() {
                    try {
                        ...
                        File signatureFile = new File(filesDir, signatureName);
                        ...
                        while (!newFile) {
                            boolean newFile1 = signatureFile.createNewFile();
                            newFile = newFile1;
                        }

                        FileOutputStream fos = new FileOutputStream(signatureFile);
                        whiteBgBitmap.compress(Bitmap.CompressFormat.PNG, 100, fos);
                        fos.flush();
                        fos.close();
    ...
    public void ensureSignatureBitmap() {
        if (mSignatureBitmap == null) {
            ...
            mSignatureBitmapCanvas.drawBitmap(mSignatureBitmap, 0, 0, getPaint());
